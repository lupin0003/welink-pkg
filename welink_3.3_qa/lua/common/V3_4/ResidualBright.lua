...
function action_gadget_state_change(context, evt)
	ScriptLib.PrintContextLog(context, "##[ResidualBright]:物件状态发生变化"..evt.param1..evt.param2)
	if evt.param2==defs.gadget_1 and evt.param1==202 and ScriptLib.GetGroupVariableValue(context, "successed")~=1 then
		ScriptLib.PrintContextLog(context, "##[ResidualBright]:挑战已经完成")
...
function action_group_load(context, evt)
	if ScriptLib.GetGroupVariableValue(context, "successed")==1 then
		...
		ScriptLib.AddExtraGroupSuite(context, base_info.group_id, 3)
		ScriptLib.SetGroupGadgetStateByConfigId(context, base_info.group_id, defs.gadget_1, 202)
		for i=1,#gadgets do
